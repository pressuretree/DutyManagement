          ...
          <mu-form ref="form" :model="validateForm" class="mu-demo-form" label-position="top">
            <mu-form-item label="选择我的换班日期" prop="myswapdate" :rules="myddRules" >
              <mu-select v-model="validateForm.myswapdate" @change="changed" action-icon="today">
                <mu-option v-for="date,index in mydutydates" :key="date" :label="date" :value="date"></mu-option>
              </mu-select>
            </mu-form-item>
            <mu-form-item label="选择换班人员" prop="swapemp" :rules="bdRules">
              <mu-select v-model="validateForm.swapemp" full-width>
                <mu-option v-for="emp in dutyemps" :key="emp" :label="emp" :value="emp"></mu-option>
              </mu-select>
            </mu-form-item>
            <mu-form-item :label="'选择'+validateForm.swapemp+'的换班日期'" prop="swapdate" :rules="ddRules" v-if='validateForm.swapemp != "" && validateForm.swapemp != undefined'>
              <mu-select v-model="validateForm.swapdate" @change="changed" action-icon="today">
                <mu-option v-for="date,index in dutydates" :key="date" :label="date" :value="date"></mu-option>
              </mu-select>
            </mu-form-item>
